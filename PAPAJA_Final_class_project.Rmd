...
library(knitr)
library(kableExtra)
options(scipen=999) # I like this!
...
<!-- I took a point from the Code for not deleting the code chunk below. This is from the papaja template, and is not needed here. -->
...
<!-- I love the headers you used in the Method section! Headers within the Intro would also help with the topic transitions. Overall, the Method section is very well structured. -->
...
## Participants 
<!-- The percentages reported below would have been perfect for inline code. -->
...
### Stress Induction 
_Trier Social Stress Task._ The Trier Social Stress Test (TSST), adapted by @yim2015experimental was used to induce stress. During the TSST, the participant entered a room where one male and one female judge were waiting. <!-- What were the ethnicities of the judges? Could this have affected findings? --> The participants were instructed to deliver a 5-minute speech, followed by mental arithmetic in front of the judges while being videotaped. If at any moment they stopped, they were instructed to continue with the task until the five minutes were up (only two reminders were allowed). If they stopped again or remained silent for 15 seconds, the judge would instruct them to continue standing for the remainder of the time.
...
#Mean tables 
#Personally, I like spaces after commas and on both sides of =.
meanscore1<- tidy %>% 
  group_by(timepoint, measure, ethnicity) %>%
  summarize(meanscores = mean(value), # I like each variable to be on a new line.
            sdscores = sd(value))
...
sleep_stress3 <- sleep_stress2 %>% 
  drop_na(bpm_response_1_2,
          bpm_recovery_2_3, 
          hrv_response_1_2_w, 
          hrv_response_2_3_w)
...
sleep_tidy %>% 
  summarize(m_mean_sleep_efficiency_c = mean(mean_sleep_efficiency_c, na.rm = TRUE),
           ...
           m_mean_sleep_dur_mi_c = mean(mean_sleep_dur_mi_c, na.rm = TRUE),
           m_mean_sleep_var_min_c = mean(mean_sleep_var_min_c, na.rm = TRUE)) %>% 
  round(5) #Did this like worked as you wanted?
...
hrvtsst_inlinecode <- meanscore100 %>%  
  filter(timepoint=="tsst", 
         measure=="hrv")

hrvrecovery_inlinecode<- meanscore100 %>%  
  filter(timepoint =="recovery", 
         measure=="hrv")

bpmbaseline_inlinecode <- meanscore100 %>%
...
```
<!-- Wait until the third course in the data science sequence, when you learn about list columns. It will completely change how you write the previous two code chunks - you'll really appreciate it.  -->
...
<!-- Ideally, all numbers reported in the results would use inline code, so you don't have to change them manually if data changes. -->
...
\newpage
<!-- This table is a perfect candidate for pivto_wider (see below). I took a point off for this. Also, either define 'tsst' or rename it for the reader. -->
```{r results='asis'}
desc <- meanscore100 %>% 
  pivot_wider(
    names_from = measure,
    values_from = c(meanscores, meansd)
  ) 

apa_table(
  desc,
  col.names = c("Time Point", "BPM", "HRV", "BPM", "HRV"),
  caption = "Mean scores and standard deviations across timepoints",
  col_spanners = list(Mean = c(2, 3), SD = c(4, 5))
)
...
<!-- These are good figures, but they could have been faceted to show both in one figure. It would also have been nice to see the distributions of these continuous variables, perhaps using geom_violin or  geom_density_ridges. I took one point off here. -->
